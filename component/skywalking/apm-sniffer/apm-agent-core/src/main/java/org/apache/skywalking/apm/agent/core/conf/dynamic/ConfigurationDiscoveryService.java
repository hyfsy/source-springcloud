...
/**
 * Watcher的配置管理，动态从oap服务器上拉取新的配置
 *
 * 内部存在注册表，注册所有的Watcher
 */
@DefaultImplementor
    ...
    private void getAgentDynamicConfig() {
        ...
        if (GRPCChannelStatus.CONNECTED.equals(status)) {
            try {
                ...
                builder.setService(Config.Agent.SERVICE_NAME); // OAP服务器
                if (null != uuid) {
                ...
                if (configurationDiscoveryServiceBlockingStub != null) {
                    ...
                    // 放入 CommandService 会转发到 this.handleConfigurationDiscoveryCommand
